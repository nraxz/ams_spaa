...
$old_error_handler = set_error_handler("user_activation_scriptcase_error_handler");

function user_activation_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['user_activation']['contr_erro']) && $_SESSION['scriptcase']['user_activation']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_user_activation->contr_user_activation->Apl_com_erro = true;
            if ($inicial_user_activation->contr_user_activation->NM_ajax_flag)
            {
                $inicial_user_activation->contr_user_activation->NM_ajax_info['errList']['geral_user_activation'][] = $msg;
                return;
            }
            user_activation_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function user_activation_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['user_activation'])  && '' != $_SESSION['scriptcase']['error_icon']['user_activation'])  ? $_SESSION['scriptcase']['error_icon']['user_activation']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['user_activation']) && '' != $_SESSION['scriptcase']['error_close']['user_activation']) ? $_SESSION['scriptcase']['error_close']['user_activation'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['user_activation']) && '' != $_SESSION['scriptcase']['error_span_title']['user_activation']) ? $_SESSION['scriptcase']['error_span_title']['user_activation'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['user_activation']) && '' != $_SESSION['scriptcase']['error_icon_title']['user_activation']) ? $_SESSION['scriptcase']['error_icon_title']['user_activation'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['user_activation']['error_buffer'] .= ob_get_flush();
}
