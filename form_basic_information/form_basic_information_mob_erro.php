...
$old_error_handler = set_error_handler("form_basic_information_mob_scriptcase_error_handler");

function form_basic_information_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_basic_information_mob']['contr_erro']) && $_SESSION['scriptcase']['form_basic_information_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_basic_information_mob->contr_form_basic_information_mob->Apl_com_erro = true;
            if ($inicial_form_basic_information_mob->contr_form_basic_information_mob->NM_ajax_flag)
            {
                $inicial_form_basic_information_mob->contr_form_basic_information_mob->NM_ajax_info['errList']['geral_form_basic_information_mob'][] = $msg;
                return;
            }
            form_basic_information_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_basic_information_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $ErrorMsgTitle   = "Error: equired field missing!";
        $sTitErr         = (!isset($ErrorMsgTitle) || '' == $ErrorMsgTitle) ? $Nm_lang['lang_errm_errt'] : $ErrorMsgTitle;
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_basic_information_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['form_basic_information_mob'])  ? $_SESSION['scriptcase']['error_icon']['form_basic_information_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_basic_information_mob']) && '' != $_SESSION['scriptcase']['error_close']['form_basic_information_mob']) ? $_SESSION['scriptcase']['error_close']['form_basic_information_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_basic_information_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['form_basic_information_mob']) ? $_SESSION['scriptcase']['error_span_title']['form_basic_information_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_basic_information_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_basic_information_mob']) ? $_SESSION['scriptcase']['error_icon_title']['form_basic_information_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_basic_information']['error_buffer'] .= ob_get_flush();
}
