...
$old_error_handler = set_error_handler("control_change_email_scriptcase_error_handler");

function control_change_email_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['control_change_email']['contr_erro']) && $_SESSION['scriptcase']['control_change_email']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_control_change_email->contr_control_change_email->Apl_com_erro = true;
            if ($inicial_control_change_email->contr_control_change_email->NM_ajax_flag)
            {
                $inicial_control_change_email->contr_control_change_email->NM_ajax_info['errList']['geral_control_change_email'][] = $msg;
                return;
            }
            control_change_email_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function control_change_email_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['control_change_email'])  && '' != $_SESSION['scriptcase']['error_icon']['control_change_email'])  ? $_SESSION['scriptcase']['error_icon']['control_change_email']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['control_change_email']) && '' != $_SESSION['scriptcase']['error_close']['control_change_email']) ? $_SESSION['scriptcase']['error_close']['control_change_email'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['control_change_email']) && '' != $_SESSION['scriptcase']['error_span_title']['control_change_email']) ? $_SESSION['scriptcase']['error_span_title']['control_change_email'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['control_change_email']) && '' != $_SESSION['scriptcase']['error_icon_title']['control_change_email']) ? $_SESSION['scriptcase']['error_icon_title']['control_change_email'] : '';
?>
