...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['control_paypal']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="control_paypal"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_paypal']['where_detal']))
  {
?>  
...
{ 
   ...
   document.F4.action               = "control_paypal_fim.php";
   document.F4.submit(); 
} 
...
function lookup_venue_id(campo)
{
   dados  = document.F1.venue_id.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('control_paypal')?>/control_paypal.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&script_case_session=<?php echo session_id() ?>&nmgp_opcao=lookup", recebe_venue_id, "venue_id", dados, false);
}
function recebe_venue_id(string)
{
   document.getElementById('id_lookup_venue_id').innerHTML = string;
}
function lookup_audition_id(campo)
{
   dados  = document.F1.audition_id.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('control_paypal')?>/control_paypal.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&script_case_session=<?php echo session_id() ?>&nmgp_opcao=lookup", recebe_audition_id, "audition_id", dados, false);
}
function recebe_audition_id(string)
{
   document.getElementById('id_lookup_audition_id').innerHTML = string;
}
<?php 
