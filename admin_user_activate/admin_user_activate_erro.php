...
$old_error_handler = set_error_handler("admin_user_activate_scriptcase_error_handler");

function admin_user_activate_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['admin_user_activate']['contr_erro']) && $_SESSION['scriptcase']['admin_user_activate']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_admin_user_activate->contr_admin_user_activate->Apl_com_erro = true;
            if ($inicial_admin_user_activate->contr_admin_user_activate->NM_ajax_flag)
            {
                $inicial_admin_user_activate->contr_admin_user_activate->NM_ajax_info['errList']['geral_admin_user_activate'][] = $msg;
                return;
            }
            admin_user_activate_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function admin_user_activate_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['admin_user_activate'])  && '' != $_SESSION['scriptcase']['error_icon']['admin_user_activate'])  ? $_SESSION['scriptcase']['error_icon']['admin_user_activate']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['admin_user_activate']) && '' != $_SESSION['scriptcase']['error_close']['admin_user_activate']) ? $_SESSION['scriptcase']['error_close']['admin_user_activate'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['admin_user_activate']) && '' != $_SESSION['scriptcase']['error_span_title']['admin_user_activate']) ? $_SESSION['scriptcase']['error_span_title']['admin_user_activate'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['admin_user_activate']) && '' != $_SESSION['scriptcase']['error_icon_title']['admin_user_activate']) ? $_SESSION['scriptcase']['error_icon_title']['admin_user_activate'] : '';
?>
