...
$old_error_handler = set_error_handler("form_applicant_grades_scriptcase_error_handler");

function form_applicant_grades_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_applicant_grades']['contr_erro']) && $_SESSION['scriptcase']['form_applicant_grades']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_applicant_grades->contr_form_applicant_grades->Apl_com_erro = true;
            if ($inicial_form_applicant_grades->contr_form_applicant_grades->NM_ajax_flag)
            {
                $inicial_form_applicant_grades->contr_form_applicant_grades->NM_ajax_info['errList']['geral_form_applicant_grades'][] = $msg;
                return;
            }
            form_applicant_grades_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_applicant_grades_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_applicant_grades'])  && '' != $_SESSION['scriptcase']['error_icon']['form_applicant_grades'])  ? $_SESSION['scriptcase']['error_icon']['form_applicant_grades']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_applicant_grades']) && '' != $_SESSION['scriptcase']['error_close']['form_applicant_grades']) ? $_SESSION['scriptcase']['error_close']['form_applicant_grades'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_applicant_grades']) && '' != $_SESSION['scriptcase']['error_span_title']['form_applicant_grades']) ? $_SESSION['scriptcase']['error_span_title']['form_applicant_grades'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_applicant_grades']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_applicant_grades']) ? $_SESSION['scriptcase']['error_icon_title']['form_applicant_grades'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_applicant_grades']['error_buffer'] .= ob_get_flush();
}
