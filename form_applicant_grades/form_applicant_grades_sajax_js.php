  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_applicant_grades" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_form_applicant_grades_form();
	}

  // ---------- Validate subject_
  function do_ajax_form_applicant_grades_validate_subject_(iNumLinha)
  {
    var nomeCampo_subject_ = "subject_" + iNumLinha;
    var var_subject_ = scAjaxGetFieldText(nomeCampo_subject_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_applicant_grades_validate_subject_(var_subject_, iNumLinha, var_script_case_init, do_ajax_form_applicant_grades_validate_subject__cb);
  } // do_ajax_form_applicant_grades_validate_subject_

  function do_ajax_form_applicant_grades_validate_subject__cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_applicant_grades_validate_subject__cb

  // ---------- Validate grades_
  function do_ajax_form_applicant_grades_validate_grades_(iNumLinha)
  {
    var nomeCampo_grades_ = "grades_" + iNumLinha;
    var var_grades_ = scAjaxGetFieldText(nomeCampo_grades_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_applicant_grades_validate_grades_(var_grades_, iNumLinha, var_script_case_init, do_ajax_form_applicant_grades_validate_grades__cb);
  } // do_ajax_form_applicant_grades_validate_grades_

  function do_ajax_form_applicant_grades_validate_grades__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "grades_" + iLineNumber;
    }
    else
    {
      sFieldValid = "grades_";
    ...
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "grades_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "grades_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_applicant_grades_validate_grades__cb

  var sc_num_ult_line = "";
  var sc_insert_open  = false;

  // ---------- add_new_line
  function do_ajax_form_applicant_grades_add_new_line(sc_clone, sc_seq_clone)
  {
    if (sc_insert_open)
    {
        if (sc_clone == 'S' && sc_seq_clone != iAjaxNewLine)
        {
          do_ajax_form_applicant_grades_cancel_insert(iAjaxNewLine);
        }
        else
        {
          return;
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn(true);
    x_ajax_form_applicant_grades_add_new_line(var_sc_clone, var_sc_seq_clone, var_sc_seq_vert, var_script_case_init, do_ajax_form_applicant_grades_add_new_line_cb);
  } // do_ajax_form_applicant_grades_add_new_line

  function do_ajax_form_applicant_grades_add_new_line_cb(sResp)
  {
    scAjaxProcOff(true);
    if ("{" == sResp.substr(0, 1)) {
        oResp = scAjaxResponse(sResp);
  ...
  } // do_ajax_form_applicant_grades_add_new_line_cb

  // ---------- backup_line
  function do_ajax_form_applicant_grades_backup_line(iNumLinha)
  {
    var var_id_ = scAjaxGetFieldText("id_" + iNumLinha);
    var var_nmgp_refresh_row = iNumLinha;
    var var_nm_form_submit = document.F1.nm_form_submit.value;
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_applicant_grades_backup_line(var_id_, var_nmgp_refresh_row, var_nm_form_submit, var_script_case_init, do_ajax_form_applicant_grades_backup_line_cb);
  } // do_ajax_form_applicant_grades_backup_line

  function do_ajax_form_applicant_grades_backup_line_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    if (!scAjaxHasError())
  ...
  } // do_ajax_form_applicant_grades_backup_line_cb

  function do_ajax_form_applicant_grades_cancel_insert(iSeqVert)
  {
    var oTBodyOld = document.getElementById("hidden_bloco_0").tBodies[0];
  ...
  } // do_ajax_form_applicant_grades_cancel_insert

  function do_ajax_form_applicant_grades_cancel_update(iSeqVert)
  {
    do_ajax_form_applicant_grades_backup_line(iSeqVert);
...
?>
  } // do_ajax_form_applicant_grades_cancel_update

  function do_ajax_form_applicant_grades_restore_buttons()
  {
<?php
...
?>
  } // do_ajax_form_applicant_grades_restore_buttons

  // ---------- table_refresh
  function do_ajax_form_applicant_grades_table_refresh()
  {
    ...
    scAjaxProcOn();
    x_ajax_form_applicant_grades_table_refresh(var_id_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_applicant_grades_table_refresh_cb);
  } //  do_ajax_form_applicant_grades_table_refresh

  function do_ajax_form_applicant_grades_table_refresh_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scJQGeneralAdd();
    scAjaxAlert(do_ajax_form_applicant_grades_table_refresh_cb_after_alert);
  } // do_ajax_form_applicant_grades_table_refresh_cb
  function do_ajax_form_applicant_grades_table_refresh_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxSetNavStatus("b");
    sc_insert_open = false;
  } // do_ajax_form_applicant_grades_table_refresh_cb_after_alert
...
function scJs_sweetalert_params(params) {
  ...
  var sc_num_ult_opc  = "";
  var sc_num_ult_tr   = "";
  function do_ajax_form_applicant_grades_submit_form(iNumLinha, indexTR)
  {
    if (scEventControl_active(iNumLinha)) {
      setTimeout(function() { do_ajax_form_applicant_grades_submit_form(iNumLinha, indexTR); }, 500);
      return;
    }
    ...
    var var_grades_ = scAjaxGetFieldText("grades_" + iNumLinha);
    var var_nm_form_submit = document.F1.nm_form_submit.value;
...
?>
    x_ajax_form_applicant_grades_submit_form(var_subject_, var_grades_, var_nmgp_refresh_row, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_applicant_grades_submit_form_cb);
  } // do_ajax_form_applicant_grades_submit_form

  function do_ajax_form_applicant_grades_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
      ...
      scAjaxHideErrorDisplay("grades_" + sc_num_ult_line);
<?php
if (isset($this->Embutida_form) && $this->Embutida_form) {
?>
      scAjaxSetReadonly();
...
?>
    ...
    scAjaxSetLabel(true);
    scAjaxSetMaster();
    scAjaxAlert(do_ajax_form_applicant_grades_submit_form_cb_after_alert);
  } // do_ajax_form_applicant_grades_submit_form_cb
  function do_ajax_form_applicant_grades_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_applicant_grades_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_applicant_grades_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    if (sc_insert_open)
    {
        do_ajax_form_applicant_grades_cancel_insert(sc_num_ult_line);
    }
    nm_uncheck_delete();
    scAjaxHideMessage();
    scAjaxHideErrorDisplay("table");
    for (iNavForm = 1; iNavForm < <?php echo $this->sc_max_reg; ?> + 1; iNavForm++)
    {
      ...
      scAjaxHideErrorDisplay("grades_" + iNavForm);
    }
    var var_id_ = document.F2.id_.value;
    var var_nm_form_submit = document.F2.nm_form_submit.value;
    var var_nmgp_opcao = document.F2.nmgp_opcao.value;
    ...
    var var_script_case_init = document.F2.script_case_init.value;
    scAjaxProcOn();
    x_ajax_form_applicant_grades_navigate_form(var_id_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_applicant_grades_navigate_form_cb);
  } // do_ajax_form_applicant_grades_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_applicant_grades']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_applicant_grades_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scErrorLineReset();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_applicant_grades_navigate_form_cb_after_alert);
  } // do_ajax_form_applicant_grades_navigate_form_cb
  function do_ajax_form_applicant_grades_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
?>
  ...
  } // do_ajax_form_applicant_grades_navigate_form_cb_after_alert
  function sc_hide_form_applicant_grades_form()
  {
...
?>
  function ajax_create_tables(iNumLinha)
  {
    ...
    iTotCampos++;
    ajax_field_list[iTotCampos] = "grades_" + iNumLinha;
    iTotCampos++;
    ...
    ajax_error_list["grades_" + iNumLinha] = {"label": "Grade<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_field_mult["subject_"][iNumLinha] = "subject_" + iNumLinha;
    ajax_field_mult["grades_"][iNumLinha] = "grades_" + iNumLinha;
    ajax_field_id["subject_" + iNumLinha] = new Array("hidden_field_label_subject_", "hidden_field_data_subject_" + iNumLinha);
    ajax_field_id["grades_" + iNumLinha] = new Array("hidden_field_label_grades_", "hidden_field_data_grades_" + iNumLinha);
    ajax_error_count["subject_" + iNumLinha] = "off";
    ajax_error_count["grades_" + iNumLinha] = "off";
<?php
if (!$this->Grid_editavel)
{
?>
    ajax_read_only["subject_" + iNumLinha] = "off";
    ajax_read_only["grades_" + iNumLinha] = "off";
<?php
}
else
{
?>
    ...
    ajax_read_only["grades_" + iNumLinha] = "on";
<?php
}
?>
  }
  function ajax_destroy_tables(iNumLinha)
  {
    ...
    ajax_error_list["grades_" + iNumLinha] = null;
  }
  ...
  function scRemoveErrors()
  {
    for (iNumLinha = 1; iNumLinha < Lim_linhas; iNumLinha++)
    {
      ...
      ajax_error_list["grades_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["grades_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["grades_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["grades_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["grades_" + iNumLinha]["onfocus"] = new Array();
    }
  }
...
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['grades_'])) ? $this->nmgp_cmp_readonly['grades_'] : 'off';
?>
    scAjaxFieldRead("grades_" + iSeq, "<?php echo $NM_contr_readonly ?>");
  }

  function mdCloseObjects(iSeq)
  {
    scAjaxFieldRead("subject_" + iSeq, "on");
    scAjaxFieldRead("grades_" + iSeq, "on");
    rerunHeaderDisplay = 1;
    scSetFixedHeaders(true);
  }
...
?>
  ...
  function scErrorLineOn(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("on" == ajax_error_count["subject_" + iRow] || "on" == ajax_error_count["grades_" + iRow]))
    {
      ...
      $("#hidden_field_data_grades_" + iRow).addClass("scFormErrorLine");
    }
  }
  ...
  function scErrorLineOff(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("off" == ajax_error_count["subject_" + iRow] && "off" == ajax_error_count["grades_" + iRow]))
    {
      if (bErrorRow)
      {
        ajax_error_count["subject_" + iRow] = "off";
        ajax_error_count["grades_" + iRow] = "off";
      }
      ...
      $("#hidden_field_data_grades_" + iRow).removeClass("scFormErrorLine");
    }
  }
