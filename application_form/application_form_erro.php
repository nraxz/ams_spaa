...
$old_error_handler = set_error_handler("application_form_scriptcase_error_handler");

function application_form_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['application_form']['contr_erro']) && $_SESSION['scriptcase']['application_form']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_application_form->contr_application_form->Apl_com_erro = true;
            if ($inicial_application_form->contr_application_form->NM_ajax_flag)
            {
                $inicial_application_form->contr_application_form->NM_ajax_info['errList']['geral_application_form'][] = $msg;
                return;
            }
            application_form_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function application_form_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $ErrorMsgTitle   = "Error: required field missing!";
        $sTitErr         = (!isset($ErrorMsgTitle) || '' == $ErrorMsgTitle) ? $Nm_lang['lang_errm_errt'] : $ErrorMsgTitle;
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['application_form'])  && '' != $_SESSION['scriptcase']['error_icon']['application_form'])  ? $_SESSION['scriptcase']['error_icon']['application_form']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['application_form']) && '' != $_SESSION['scriptcase']['error_close']['application_form']) ? $_SESSION['scriptcase']['error_close']['application_form'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['application_form']) && '' != $_SESSION['scriptcase']['error_span_title']['application_form']) ? $_SESSION['scriptcase']['error_span_title']['application_form'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['application_form']) && '' != $_SESSION['scriptcase']['error_icon_title']['application_form']) ? $_SESSION['scriptcase']['error_icon_title']['application_form'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['application_form']['error_buffer'] .= ob_get_flush();
}
