  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_files_upload_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_form_files_upload_mob_form();
	}

  // ---------- Validate how_to
  function do_ajax_form_files_upload_mob_validate_how_to()
  {
  } // do_ajax_form_files_upload_mob_validate_how_to

  function do_ajax_form_files_upload_mob_validate_how_to_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_files_upload_mob_validate_how_to_cb

  // ---------- Validate image_upload
  function do_ajax_form_files_upload_mob_validate_image_upload()
  {
    var nomeCampo_image_upload = "image_upload";
    var var_image_upload = scAjaxGetFieldText(nomeCampo_image_upload);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_files_upload_mob_validate_image_upload(var_image_upload, var_script_case_init, do_ajax_form_files_upload_mob_validate_image_upload_cb);
  } // do_ajax_form_files_upload_mob_validate_image_upload

  function do_ajax_form_files_upload_mob_validate_image_upload_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "image_upload";
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_files_upload_mob_validate_image_upload_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_form_files_upload_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_form_files_upload_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_how_to = scAjaxGetFieldHidden("how_to");
    var var_image_upload = "";
    var var_login = scAjaxGetFieldText("login");
    var var_ul_info_image_upload = scJQMultiUploadPrepare_image_upload();
    ...
    scAjaxProcOn();
    x_ajax_form_files_upload_mob_submit_form(var_how_to, var_image_upload, var_login, var_ul_info_image_upload, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_files_upload_mob_submit_form_cb);
  } // do_ajax_form_files_upload_mob_submit_form

  function do_ajax_form_files_upload_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    ...
    if (scAjaxIsOk())
    {
      ...
      scAjaxHideErrorDisplay("table");
      scAjaxHideErrorDisplay("how_to");
      scAjaxHideErrorDisplay("image_upload");
      scLigEditLookupCall();
<?php
...
?>
    ...
    scClearUploadQueue_image_upload('queue');
      scUploadQueue_image_upload = new Array();
      $("#id_sc_upload_done_image_upload" ).html("");
      $("#id_sc_upload_todo_image_upload" ).html("");
    }
    scAjaxShowDebug();
    scAjaxSetDisplay();
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_form_files_upload_mob_submit_form_cb_after_alert);
  } // do_ajax_form_files_upload_mob_submit_form_cb
  function do_ajax_form_files_upload_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_files_upload_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_files_upload_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    scAjaxProcOn();
    x_ajax_form_files_upload_mob_navigate_form(var_login, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_files_upload_mob_navigate_form_cb);
  } // do_ajax_form_files_upload_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_files_upload_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_files_upload_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxClearErrors()
    sc_mupload_ok = true;
    scClearUploadQueue_image_upload('all');
    scAjaxSetFields(false);
    scAjaxSetVariables();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_files_upload_mob_navigate_form_cb_after_alert);
  } // do_ajax_form_files_upload_mob_navigate_form_cb
  function do_ajax_form_files_upload_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_form_files_upload_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_form_files_upload_mob_navigate_form_cb_after_alert
  function sc_hide_form_files_upload_mob_form()
  {
  ...
  function scClearUploadQueue_image_upload(clearMode)
  {
     if ("all" == clearMode)
     {
        $("#id_sc_upload_done_image_upload").html("");
        $("#id_sc_upload_todo_image_upload").html("");
        $("#id_sc_loaded_image_upload").html("");
     }
     scUploadQueue_image_upload.length = 0;
     var $elemSubmit = $("#id_sc_submit_image_upload");
  ...
  ajax_field_list[0] = "how_to";
  ajax_field_list[1] = "image_upload";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "how_to": {"label": "How to upload Files", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5},
    "image_upload": {"label": "File Upload", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  var ajax_field_mult = {
    "how_to": new Array(),
    "image_upload": new Array()
  };
  ajax_field_mult["how_to"][1] = "how_to";
  ajax_field_mult["image_upload"][1] = "image_upload";

  var ajax_field_id = {
    "how_to": new Array("hidden_field_label_how_to", "hidden_field_data_how_to"),
    "image_upload": new Array("hidden_field_label_image_upload", "hidden_field_data_image_upload")
  };
