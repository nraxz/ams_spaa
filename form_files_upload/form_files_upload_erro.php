...
$old_error_handler = set_error_handler("form_files_upload_scriptcase_error_handler");

function form_files_upload_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_files_upload']['contr_erro']) && $_SESSION['scriptcase']['form_files_upload']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_files_upload->contr_form_files_upload->Apl_com_erro = true;
            if ($inicial_form_files_upload->contr_form_files_upload->NM_ajax_flag)
            {
                $inicial_form_files_upload->contr_form_files_upload->NM_ajax_info['errList']['geral_form_files_upload'][] = $msg;
                return;
            }
            form_files_upload_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_files_upload_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_files_upload'])  && '' != $_SESSION['scriptcase']['error_icon']['form_files_upload'])  ? $_SESSION['scriptcase']['error_icon']['form_files_upload']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_files_upload']) && '' != $_SESSION['scriptcase']['error_close']['form_files_upload']) ? $_SESSION['scriptcase']['error_close']['form_files_upload'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_files_upload']) && '' != $_SESSION['scriptcase']['error_span_title']['form_files_upload']) ? $_SESSION['scriptcase']['error_span_title']['form_files_upload'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_files_upload']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_files_upload']) ? $_SESSION['scriptcase']['error_icon_title']['form_files_upload'] : '';
?>
