...
;
$maintenance_status = $this->maintenance();

if ($maintenance_status == '1'){
 
	 if (!isset($this->Campos_Mens_erro) || empty($this->Campos_Mens_erro))
 {
$this->nmgp_redireciona_form($this->Ini->path_link . "" . SC_dir_app_name('maintenance') . "/", $this->nm_location, "", "_self", "ret_self", 440, 630);
 };
...
if ($maintenance_status == '2'){
	 ...
	 if (!isset($this->Campos_Mens_erro) || empty($this->Campos_Mens_erro))
 {
$this->nmgp_redireciona_form($this->Ini->path_link . "" . SC_dir_app_name('application_close') . "/", $this->nm_location, "", "_self", "ret_self", 440, 630);
 };
...
$_SESSION['scriptcase']['app_Login']['contr_erro'] = 'off';
      ...
      if (!$run_iframe && isset($_SESSION['scriptcase']['menu_atual']) && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_Login']['sc_outra_jan']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['app_Login']['sc_outra_jan']))
      {
          ...
          if ($this->Ini->sc_page == $this->sc_init_menu && !isset($_SESSION['scriptcase']['menu_apls'][$_SESSION['scriptcase']['menu_atual']][$this->sc_init_menu]['app_Login']))
          {
               ...
               $_SESSION['scriptcase']['menu_apls'][$_SESSION['scriptcase']['menu_atual']][$this->sc_init_menu]['app_Login']['label'] = "AMS Login";
      ...
      { 
          require_once($this->Ini->path_embutida . 'login/app_Login_erro.class.php');
      }
      else
      { 
...
//
      ...
      if ($Campos_Crit != "" || !empty($Campos_Falta) || $this->Campos_Mens_erro != "" || $campos_erro != "" || !isset($this->bok) || $this->bok != "OK" || $this->nmgp_opcao == "recarga")
      {
          if ($Campos_Crit == "" && empty($Campos_Falta) && $this->Campos_Mens_erro == "" && !isset($this->bok) && $this->nmgp_opcao != "recarga")
          {
              ...
              { 
                  $pswd = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login']['campos'][0]; 
                  $user_email = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login']['campos'][1]; 
              } 
          }
          $this->nm_gera_html();
          $this->NM_close_db(); 
      ...
      elseif (isset($this->bok) && $this->bok == "OK")
      {
          ...
          $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login']['campos'][0] = $this->pswd; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login']['campos'][1] = $this->user_email; 
...
<HEAD>
 <TITLE><?php echo strip_tags("AMS Login") ?></TITLE>
...
$semail = $this->Db->qstr($this->user_email);
$salt = 'Iamsalt';

$spswd = $this->Db->qstr(md5(md5($salt).md5($this->pswd )));
...
if(count($this->rs ) == 0)
{
 ...
 if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= "Incorrect Email/Password";
 if ('submit_form' == $this->NM_ajax_opcao || 'event_' == substr($this->NM_ajax_opcao, 0, 6))
 {
  ...
  $this->NM_ajax_info['errList'][$sErrorIndex][] = "Incorrect Email/Password";
 }
;
}
else if($this->rs[0][2] == 'Y')
{
	$usr_email		= $this->user_email ;
	$usr_login		= $this->rs[0][0];
	$usr_priv_admin 	= ($this->rs[0][1] == 'Y') ? TRUE : FALSE;
	$usr_name		= $this->rs[0][3];	
...
;
	...
	$this->rs->Close();
		...
		;
		$_SESSION['my_session'] = time() + 1440;
		$check_group = $this->getGroup($this->sc_temp_usr_login);
	
		if ($check_group == '2'){
			 if (isset($this->sc_temp_usr_login)) { $_SESSION['usr_login'] = $this->sc_temp_usr_login;}
 if (!isset($this->Campos_Mens_erro) || empty($this->Campos_Mens_erro))
 {
$this->nmgp_redireciona_form($this->Ini->path_link . "" . SC_dir_app_name('app_menu_applicant') . "/", $this->nm_location, "", "_self", "ret_self", 440, 630);
 };
			}
 ...
 if (!isset($this->Campos_Mens_erro) || empty($this->Campos_Mens_erro))
 {
$this->nmgp_redireciona_form($this->Ini->path_link . "" . SC_dir_app_name('menu_examiner') . "/", $this->nm_location, "", "_self", "ret_self", 440, 630);
 };
			}
 ...
 if (!isset($this->Campos_Mens_erro) || empty($this->Campos_Mens_erro))
 {
$this->nmgp_redireciona_form($this->Ini->path_link . "" . SC_dir_app_name('app_menu_examiner_head') . "/", $this->nm_location, "", "_self", "ret_self", 440, 630);
 };
			}
	    else{
		 if (isset($this->sc_temp_usr_login)) { $_SESSION['usr_login'] = $this->sc_temp_usr_login;}
 if (!isset($this->Campos_Mens_erro) || empty($this->Campos_Mens_erro))
 {
$this->nmgp_redireciona_form($this->Ini->path_link . "" . SC_dir_app_name('menu_admin') . "/", $this->nm_location, "", "_self", "ret_self", 440, 630);
 };	
		}
...
$_SESSION['scriptcase']['app_Login']['contr_erro'] = 'off'; 
    ...
    function ValidateField_pswd(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
      ...
      { 
          if (NM_utf8_strlen($this->pswd) > 40) 
          { 
              $hasError = true;
              $Campos_Crit .= "Password " . $this->Ini->Nm_lang['lang_errm_mxch'] . " 40 " . $this->Ini->Nm_lang['lang_errm_nchr']; 
              if (!isset($Campos_Erros['pswd']))
              {
                  $Campos_Erros['pswd'] = array();
              }
              $Campos_Erros['pswd'][] = $this->Ini->Nm_lang['lang_errm_mxch'] . " 40 " . $this->Ini->Nm_lang['lang_errm_nchr'];
              if (!isset($this->NM_ajax_info['errList']['pswd']) || !is_array($this->NM_ajax_info['errList']['pswd']))
              {
                  $this->NM_ajax_info['errList']['pswd'] = array();
              }
              $this->NM_ajax_info['errList']['pswd'][] = $this->Ini->Nm_lang['lang_errm_mxch'] . " 40 " . $this->Ini->Nm_lang['lang_errm_nchr'];
          } 
      } 
        if ($hasError) {
            global $sc_seq_vert;
    ...
    function ValidateField_user_email(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
      ...
      if ($this->nmgp_opcao != "excluir") 
      { 
          if (NM_utf8_strlen($this->user_email) > 50) 
          { 
              $hasError = true;
              ...
              $Campos_Erros['user_email'][] = $this->Ini->Nm_lang['lang_errm_mxch'] . " 50 " . $this->Ini->Nm_lang['lang_errm_nchr'];
              ...
              $this->NM_ajax_info['errList']['user_email'][] = $this->Ini->Nm_lang['lang_errm_mxch'] . " 50 " . $this->Ini->Nm_lang['lang_errm_nchr'];
          } 
      } 
        if ($hasError) {
            global $sc_seq_vert;
